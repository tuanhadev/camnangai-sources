...
- Plan systems that grow with user demand
- Create reliable deployment and testing pipelines
- Anticipate unusual but critical failure cases
- Make smart tradeoffs between performance and cost
- Find non-obvious security weaknesses
...
AI can, for instance, be great at passing simple prompts to create solutions to well known problems. Here's an example of code you might get for a simple JSON validator:
...
For more complex systems like a Forex Trading platform, AI falls short. Even breaking it down to a smaller subset — such as a FIX execution report endpoint — gets closer but is far from a complete solution:
...
But this does not come close to the final version. Even if you could use this code as a part of your trading system, unless you know how to build and architect an entire software system, where do you go from here?
...
I'm yet to see any proof anywhere that anyone has built such a complex system without the oversight of a human, and I'm not convinced we will see it at any point in the near future.
...
A good example of something that AI is perfect for is translation logic. For instance, converting between a Python Dataclass and a Django ORM representation of the same entity:
...
AI can generate a conversion function that even accounts for field name mismatches:
...
from mydataclasses import InternalUser as DCInternalUser
...
You can even ask AI to generate test function stubs:
...
You should build expertise in **infrastructure and deployment processes**. Software that works in development but fails in production creates no value. Learn about continuous integration, monitoring systems, and cloud platform capabilities.
